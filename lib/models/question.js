...
function Question (opt) {
  ...
  this.time = Number(opt.difficulty) * 180;
  this.type = opt.type;
...
Question.findAll = function (fn) {
  questions.find().toArray(function (err, records) {
    if (err) {
      return fn(err);
    }
...
Question.total = function (fn) {
  questions.find().toArray(function (err, records) {
    if (err) {
      return fn(err);
    }

    fn(null, records.length);
  });
}
